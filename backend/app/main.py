...
from app.routers import auth, products, inventory, events, forecast, orders
...
async def lifespan(app: FastAPI):
    ...
    # Retry DB connection — MySQL needs a few seconds after healthcheck passes
    max_retries = 10
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"DB not ready (attempt {attempt + 1}/{max_retries}), retrying in 3s... {e}")
                time.sleep(3)
            else:
